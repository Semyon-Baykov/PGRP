...
function CITYWORKER.Begin( ply, ent )
    if ply:Team() != TEAM_CITYWORKER then
        ply:ChatAddText( Color( 63, 79, 127 ), '[ЖКХ] ', Color( 255, 255, 255 ), CITYWORKER.Config.Language["NOT_CITY_WORKER"] )
        --DarkRP.notify( ply, 1, 4, CITYWORKER.Config.Language["NOT_CITY_WORKER"] )
        return
    end

    if ent.CW_isWorked then
        ply:ChatAddText( Color( 63, 79, 127 ), '[ЖКХ] ', Color( 255, 255, 255 ), CITYWORKER.Config.Language["JOB_WORKED"] )
        --DarkRP.notify( ply, 1, 4, CITYWORKER.Config.Language["JOB_WORKED"] )
        return
    end

    if ply.CW_ent != ent then
        ply:ChatAddText( Color( 63, 79, 127 ), '[ЖКХ] ', Color( 255, 255, 255 ), CITYWORKER.Config.Language["ASSIGNED_ELSE"] )
...
function CITYWORKER.Finish( ply, ent )
    ...
    ply:ChatAddText( Color( 63, 79, 127 ), '[ЖКХ] ', Color( 255, 255, 255 ), 'Вы заработали '..DarkRP.formatMoney( pay )..'!' )
...
function CITYWORKER.Cancel( ply, ent )
    ...
    ply:ChatAddText( Color( 63, 79, 127 ), '[ЖКХ] ', Color( 255, 255, 255 ), CITYWORKER.Config.Language["CANCELLED"] )
end
...
function CITYWORKER.Assign( ply, job, id )
    ...
    ply:ChatAddText( Color( 63, 79, 127 ), '[ЖКХ] ', Color( 255, 255, 255 ), CITYWORKER.Config.Language["NEW_JOB"] )
end
...
function CITYWORKER.Find()
    ...
    --print(availableJobs[id])
    local job = CITYWORKER_DATA[game.GetMap()][id]
    --print(CITYWORKER_DATA[game.GetMap()][id])
...
function CITYWORKER.Initialize()
    ...
    for k, v in pairs( CITYWORKER_DATA[game.GetMap()] ) do
        ...
        -- Using this, we get approximately 76561198051291876 results.
...
net.Receive( "CITYWORKER.Add", function( len, ply )
    ...
    if CITYWORKER_DATA[game.GetMap()] and #CITYWORKER_DATA[game.GetMap()] > 255 then 
