...
function SWEP:Initialize()
	...
	self:SetHolsterStatus(false)
	self:SetHolsterTimer(0.5)
...
function SWEP:PrimaryAttack()
	if self:CanPrimaryAttack() and self.Owner:IsPlayer() and not self.Owner.WepHolstered then
...
function SWEP:Reload()
	...
	if self:GetHolsterStatus() then return end
	...
	self:SetHolsterTimer((self.Owner:GetViewModel():SequenceDuration()) + 0.2)
end
...
function SWEP:TossOrHolster()

	--if (gmod.GetGamemode().Name == "Murderthon 9000") or GetConVar("DebugM9K"):GetBool() then
	//everything you need to make a weapon-tossing function goes here
	--end
	
	if (GetConVar("M9KDisableHolster"):GetBool()) then return end
	
	if self.NextHolst > CurTime() then return end
	if (self.Weapon:GetNWBool("Reloading")) then return end
	
	self:SetHolsterTimer(0.5)
	
	if self:GetHolsterStatus() then
		self.Owner:SetFOV( 0, 0.3 )
		self.DrawCrosshair = self.OrigCrossHair
		self:SetIronsights(false, self.Owner)
		self:SetHolsterStatus(false)
		self.Weapon:SetNextPrimaryFire(CurTime()+0.3)
		if CLIENT or SERVER then self:SetWeaponHoldType(self.HoldType) end
	else
		self.IronSightsPos = self.HolsterPos					-- Hold it down
		self.IronSightsAng = self.HolsterAng					-- Hold it down
		self:SetIronsights(true, self.Owner)					-- Set the ironsight true
		self.Owner:SetFOV( 0, 0.3 )
		self.DrawCrosshair = false
		self:SetHolsterStatus(true)
		if CLIENT or SERVER then 
			if self.HoldType == "smg" or 
			self.HoldType == "ar2" or 
			self.HoldType == "rpg" or 
			self.HoldType == "physgun" or 
			self.HoldType == "shotgun" or 
			self.HoldType == "crossbow" or 
...
function SWEP:GetHolsterStatus()
	return (self.Owner:GetNWBool("WepHolstered"))
end

function SWEP:SetHolsterStatus(bool)
	self.Owner:SetNWBool("WepHolstered", bool)
	self.Owner.WepHolstered = bool
end

function SWEP:SetHolsterTimer(num)
	self.NextHolst = (CurTime() + num)
end
...
function SWEP:IronSight()
	...
	--===============================everything up above here is just fine for holstered or not
	
	if self.Owner:KeyDown(IN_USE) and self.Owner:KeyPressed(IN_WALK) then
		self:TossOrHolster()	
	end
	
	if self.Owner.WepHolstered then return end
