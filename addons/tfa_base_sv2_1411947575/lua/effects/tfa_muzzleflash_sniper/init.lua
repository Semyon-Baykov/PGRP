...
function EFFECT:Init(data)
	...
	if (sparticle) then
		...
		--sparticle:SetStartSize( 8 * (halofac*0.8+0.2), 0, 1)
		--sparticle:SetEndSize( 0 )
		sparticle:SetStartSize(4 * (halofac * 0.8 + 0.2), 0, 1)
		sparticle:SetEndSize(8 * (halofac * 0.8 + 0.2), 0, 1)
	...
	local sparkcount = math.random(4, 5)

	for i = 0, sparkcount do
		local particle = emitter:Add("effects/yellowflare", self.Position)

		if (particle) then
			particle:SetVelocity((VectorRand() + Vector(0, 0, 0.3)) * 40 * Vector(0.8, 0.8, 0.6) + dir * math.Rand(45, 60) + 1.15 * AddVel)
			particle:SetLifeTime(0)
