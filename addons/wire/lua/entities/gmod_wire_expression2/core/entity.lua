...
e2function void setMass(mass)
	...
	if not self.player:E2_FullAccess() then
		self.player:ChatAddText( Color(255, 0, 0), '[Ошибка] ', Color(255,255,255), 'У вас нету доступа к setMass функциии.' )
		return nil
	end
...
e2function void entity:setMaterial(string material)
	if not self.player:E2_FullAccess() then
		self.player:ChatAddText( Color(255, 0, 0), '[Ошибка] ', Color(255,255,255), 'У вас нету доступа к material функциям.' )
		return false
	end
	if not IsValid(this) then return end
	if not isOwner(self, this) then return end
...
e2function void entity:setSkin(skin)
	if not self.player:E2_FullAccess() then
		self.player:ChatAddText( Color(255, 0, 0), '[Ошибка] ', Color(255,255,255), 'У вас нету доступа к entity функциям.' )
		return false
	end
...
e2function void entity:applyForce(vector force)
	...
	if not self.player:E2_FullAccess() then
		self.player:ChatAddText( Color(255, 0, 0), '[Ошибка] ', Color(255,255,255), 'У вас нету доступа к Force функциям.' )
		return false
	end
	if check( force ) then
		local phys = this:GetPhysicsObject()
