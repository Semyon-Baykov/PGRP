...
function ENT:Draw()
	...
	local potTime = "Прогресс: "..self:GetNWInt("progress").."% (Потряси!)";
	
	if (self:GetNWInt("status") == "inprogress") then
		potTime = "Прогресс: "..self:GetNWInt("progress").."% (Потряси!)";
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < self:GetNWInt("distance") then
		...
		cam.Start3D2D(pos + ang:Up()*5, ang, 0.055)
			draw.SimpleTextOutlined("Кристаллизированный йод", "methFont", 0, -56, Color(220, 134, 159, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
			...
			draw.SimpleTextOutlined("Ингредиенты", "methFont", -101, 8, Color(220, 134, 159, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
		...
		cam.Start3D2D(pos + ang:Up()*5, ang, 0.045)		
			draw.SimpleTextOutlined("Соляная кислота ("..self:GetNWInt("macid")..")", "methFont", -121, 44, macidColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
