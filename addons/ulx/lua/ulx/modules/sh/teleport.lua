...
function ulx.adminr( calling_ply, target_ply )
	if not calling_ply:IsValid() then
		Msg( "You are the console, you can't teleport or teleport others since you can't see the world!\n" )
		return
	end
	...
	target_ply:SetPos( Vector(-9982.792969, 7289.799316 ,5059.551758) + Vector( math.random( 0, 1 ) and 75 or 0, math.random( 0, 1 ) and 75 or 0, 5  ) )
	target_ply:SetLocalVelocity( Vector( 0, 0, 0 ) ) -- Stop!

	if target_ply ~= calling_ply then
		ulx.fancyLogAdmin( calling_ply, "#A teleported #T in admin room", target_ply ) -- We don't want to log otherwise
	end
end
local teleport = ulx.command( CATEGORY_NAME, "ulx adminr", ulx.adminr, {"!adminr", "!adminroom"} )
teleport:addParam{ type=ULib.cmds.PlayerArg, ULib.cmds.optional }
teleport:defaultAccess( ULib.ACCESS_ADMIN )
teleport:help( "Телепортирует в админ-рум" )
