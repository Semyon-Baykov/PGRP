...
function ENT:Use( ply, caller )
	if caller:IsPlayer() and ( self.lastUsed or CurTime() ) <= CurTime() then
		...
		if caller:Armor() + 50 >= CH_AdvMedic.Config.MaximumArmor then
			...
			caller:SetArmor( caller:Armor() + 50 )
			DarkRP.notify( caller, 1, 5, "+50 ".. CH_AdvMedic.Config.Lang["Armor"][CH_AdvMedic.Config.Language] )
		end
		self:Remove()
	end
end