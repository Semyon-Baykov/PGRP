...
function ENT:Use( ply, caller )
	if caller:IsPlayer() and ( self.lastUsed or CurTime() ) <= CurTime() then
		...
		if caller:Health() + 50 >= caller:GetMaxHealth() then
			...
			caller:SetHealth( caller:Health() + 50 )
			DarkRP.notify( caller, 1, 5,  "+50 ".. CH_AdvMedic.Config.Lang["Health"][CH_AdvMedic.Config.Language] )
		end
