...
function ADV_MEDIC_DEFIB_HUDPaint()
	...
	if not LocalPlayer():Alive() and CH_AdvMedic.NextRespawn then
		if CH_AdvMedic.Config.BecomeDarkerWhenDead then
			alpha = math.Clamp( alpha + 5, 0, CH_AdvMedic.Config.DarkestAlpha )
			
			surface.SetDrawColor( 0, 0, 0, alpha )
			surface.DrawRect( 0, 0, ScrW(), ScrH() )
		end
		
		if CH_AdvMedic.NextRespawn - CurTime() > 0 then -- Если вы хотите вызвать администратора, напишите: /// [жалоба]
			draw.SimpleText( CH_AdvMedic.Config.Lang["You have died"][CH_AdvMedic.Config.Language], "RespawnTimer", ScrW() / 2, (ScrH() / 2) - ScrH() * 0.1, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			draw.SimpleText( CH_AdvMedic.Config.Lang["Paramedics can rescue you using defibrillators"][CH_AdvMedic.Config.Language], "RespawnTimer", ScrW() / 2, (ScrH() / 2) - ScrH() * 0.06, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			draw.SimpleText( "Если вы хотите вызвать администратора, напишите: /// [жалоба]", "RespawnTimer", ScrW() / 2, (ScrH() / 2) - ScrH() * 0.04, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			draw.SimpleText( CH_AdvMedic.Config.Lang["Please wait"][CH_AdvMedic.Config.Language] .." ".. math.Round( CH_AdvMedic.NextRespawn - CurTime() ) .." ".. CH_AdvMedic.Config.Lang["seconds to respawn"][CH_AdvMedic.Config.Language], "RespawnTimer", ScrW() / 2, (ScrH() / 2) - ScrH() * 0.02, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		else
			if CH_AdvMedic.Config.ClickToRespawn or LocalPlayer():gp_VipAccess() then 
				draw.SimpleText( CH_AdvMedic.Config.Lang["Click to respawn"][CH_AdvMedic.Config.Language], "RespawnTimer", ScrW() / 2, ScrH() / 2, color_white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			end
        end
	end
end
