...
function ENT:Use( ply )
	...
	if IsEntity(self.lastowner) and self.lastowner == ply then
		self:keysUnOwn( self:getDoorOwner() )
		self:keysOwn( ply )
		DarkRP.notify( ply, 2, 4, 'Вы вернули ваше угнанное т/c.' )
		self.lastowner = nil
		self:SetNWInt( 'veh_lastowner', '' )
		self:CPPISetOwner(world)
		return
	end

	if isstring(self.lastowner) and string.len( self.lastowner ) > 0 and table.HasValue(RPExtraTeamDoors[self.lastowner] or {}, ply:Team()) then
		self:keysUnOwn( self:getDoorOwner() )
		self:setDoorGroup( self.lastowner )
		self.lastowner = nil
		self:CPPISetOwner(world)
		self:SetNWInt( 'veh_lastowner', '' )
		DarkRP.notify( ply, 2, 4, 'Вы вернули ваше угнанное т/c.' )
...
function ENT:SetPassenger( ply )
	...
	if not IsValid(self:GetDriver()) and not ply:KeyDown(IN_WALK) and ply:gpv_checkaccess( self ) then
