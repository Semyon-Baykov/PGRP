...
--[[
local var = SG.NW.Register 'MyVar' 	-- You MUST call this ALL shared
	...
	var:SetGlobal() 				-- Registers the var for use with SG.NW.SetGlobal
...
SG.NW.WaitForPlayer(player, callback) 	-- Calls your callback when the player is ready to recieve net messages

-- Set Functions
ENTITY:SetNetVar(var, value)
SG.NW.SetGlobal(var, value)
...
if SG.NW then return end -- How to not lose everyone's networking
...
function SG.NW.Register(var) -- You must always call this on both the client and server. It will serioulsy break shit if you don't.
	local t = {
		Name = var,
		NetworkString = 'SG.NW_' .. var,
...
if (SERVER) then
	util.AddNetworkString 'SG.NW.PlayerSync'
	util.AddNetworkString 'SG.NW.NilEntityVar'
	util.AddNetworkString 'SG.NW.NilPlayerVar'
	util.AddNetworkString 'SG.NW.EntityRemoved'
	util.AddNetworkString 'SG.NW.PlayerRemoved'

	net.Receive('SG.NW.PlayerSync', function(len, pl)
	...
	function SG.NW.WaitForPlayer(pl, cback)
		if (pl.EntityCreated == true) then
			cback(pl)
		else
			if (callbacks[pl] == nil) then
	...
	function SG.NW.SetGlobal(var, value)
		globals[var] = value
		if (value ~= nil) then
			vars[var]:_Send(0, value)
		else
			net_Start('SG.NW.NilEntityVar')
				net_WriteUInt(0, 12)
	...
	hook.Add('InitPostEntity', 'SG.NW.InitPostEntity', function()
		net_Start('SG.NW.PlayerSync')
		net_Send()
	end)

	net.Receive('SG.NW.NilEntityVar', function()
	...
	net.Receive('SG.NW.NilPlayerVar', function()
	...
	net.Receive('SG.NW.EntityRemoved', function()
		data[net_ReadUInt(12)] = nil
	end)

	net.Receive('SG.NW.PlayerRemoved', function()
