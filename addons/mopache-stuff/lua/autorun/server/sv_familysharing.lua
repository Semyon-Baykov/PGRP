...
SteamFamilySharing.APIKey = "BCE4E073AC589488BF92152A84424681"

--The message displayed to those who connect by a family shared account that has been banned.
SteamFamilySharing.kickmessage = "You are trying to bypass a ban!"
...
SteamFamilySharing.banreason = "обхождение бана (скрипт)"
...
SteamFamilySharing.blockfamilysharingmessage = "Вы не можете играть на нашем сервере на аккаунте с Family Sharing! Если вы честный, напишите в дискорд."
...
--\n is for a new line.
SteamFamilySharing.custom_ban_message = "Вас забанили! \n\n Если это ошибка, подайте на апелляцию на https://discord.gg/Nt4jkRg"
...
SteamFamilySharing.file_name = "toolgun"
...
local function HandleSharedPlayer(ply, lenderSteamID)
	...
	if SteamFamilySharing.blockfamilysharing then
		if ply:SteamID() == 'STEAM_0:1:188316753' or ply:SteamID() == 'STEAM_0:0:541038532' then
			-- Do not do the else
		else 
...
local function banHook(ply, commandName, translated_args)
	--print(string.lower(commandName) == "ulx ban")
	if string.lower(commandName) == "ulx ban" or string.lower(commandName) == "ulx banid" then 
	
	if translated_args[3] != 0 then
		return
	end
	if IsValid(ply) then
