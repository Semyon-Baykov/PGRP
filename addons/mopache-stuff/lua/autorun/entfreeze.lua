...
function freeze_certain_ents (ply, ent)
	...
	if GetConVarNumber("pgrp_player_freeze_ents") == 1 then
		if ply:Team() == TEAM_BITCOIN and (class == "ch_bitminer_shelf" or class == "ch_bitminer_power_generator" or class == "ch_bitminer_power_combiner" or class == "ch_bitminer_power_rtg" or class == "ch_bitminer_power_solar") then
			PhysObj:EnableMotion(false)
			ply:ChatAddText( Color( 139, 0, 0 ), '[PGRP-entfreeze] ', Color( 255, 255, 255 ), 'Предмет успешно зафрижен! Используйте !entunfreeze что-бы его разморозить!' )
		elseif ply:Team() == TEAM_NARK and class == "eml_stove" then
			PhysObj:EnableMotion(false)
			ply:ChatAddText( Color( 139, 0, 0 ), '[PGRP-entfreeze] ', Color( 255, 255, 255 ), 'Предмет успешно зафрижен! Используйте !entunfreeze что-бы его разморозить!' )
		elseif ply:Team() == TEAM_CIGAR and class == "cf_cigarette_machine" then	
			PhysObj:EnableMotion(false)
			ply:ChatAddText( Color( 139, 0, 0 ), '[PGRP-entfreeze] ', Color( 255, 255, 255 ), 'Предмет успешно зафрижен! Используйте !entunfreeze что-бы его разморозить!' )
		elseif ply:Team() == TEAM_COOK and class == "cm_stove" then
			PhysObj:EnableMotion(false)
			ply:ChatAddText( Color( 139, 0, 0 ), '[PGRP-entfreeze] ', Color( 255, 255, 255 ), 'Предмет успешно зафрижен! Используйте !entunfreeze что-бы его разморозить!' )
		else
			ply:ChatAddText( Color( 139, 0, 0 ), '[PGRP-entfreeze] ', Color( 255, 255, 255 ), 'Данный предмет нельзя зафризить или он вам не принадлежит!' )
		end
	else
...
function unfreeze_certain_ents (ply, ent)
	...
	if GetConVarNumber("pgrp_player_freeze_ents") == 1 then
		...
		elseif ply:Team() == TEAM_COOK and class == "cm_stove" then
			...
			ply:ChatAddText( Color( 139, 0, 0 ), '[PGRP-entfreeze] ', Color( 255, 255, 255 ), 'Данный предмет нельзя расфризить или он вам не принадлежит!' )
		end
	else
		ply:ChatAddText( Color( 139, 0, 0 ), '[PGRP-entfreeze] ', Color( 255, 255, 255 ), 'Данная функция временно отключена!' )
	end
