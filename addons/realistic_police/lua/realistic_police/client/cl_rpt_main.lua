...
local FontTable = {}
function Realistic_Police.Fonts(SizeId)
...
local function CreateNumSlider(id, x, y, panel, min, max)
    local TableSlider = {
        [1] = Realistic_Police.Lang[23][Realistic_Police.Langage],
        [2] = Realistic_Police.Lang[24][Realistic_Police.Langage],
        [3] = Realistic_Police.Lang[25][Realistic_Police.Langage],
        [4] = Realistic_Police.Lang[26][Realistic_Police.Langage],
        [5] = Realistic_Police.Lang[27][Realistic_Police.Langage],
        [6] = Realistic_Police.Lang[28][Realistic_Police.Langage],
        [7] = Realistic_Police.Lang[30][Realistic_Police.Langage],
        [8] = Realistic_Police.Lang[31][Realistic_Police.Langage],
    }
    ...
    timer.Simple(Timer, function()
        ...
        for k,v in pairs(Realistic_Police.Application) do 
            ...
            Buttons.DoClick = function()
                ...
                if Realistic_Police.OpenComputer[team.GetName(LocalPlayer():Team())] then 
                ...
                if Realistic_Police.HackerJob[team.GetName(LocalPlayer():Team())] && RPTEntComputer:GetNWBool("rpt_hack") or v.Type == "hacker" then 
                    v.Function(RPTFrame, RPTEntComputer)
                elseif not Realistic_Police.OpenComputer[team.GetName(LocalPlayer():Team())] then 
                    RealisticPoliceNotify(Realistic_Police.Lang[45][Realistic_Police.Langage])
                end 
            end 
...
net.Receive("RealisticPolice:SetupVehicle", function()
    ...
    Save:SetText(string.upper(Realistic_Police.Lang[16][Realistic_Police.Langage]))
...
function Realistic_Police.PoliceTrunk()
    ...
    Frame.Paint = function(self,w,h)
        ...
        surface.DrawRect( 0, 0, w, h )
        draw.DrawText(Realistic_Police.Lang[84][Realistic_Police.Langage], "rpt_font_7", RespX*0.003, RespY*0.002, Realistic_Police.Colors["gray220"], TEXT_ALIGN_LEFT)
    end 
    ...
    for k,v in pairs(Realistic_Police.Trunk) do
        ...
        ListItem.DoClick = function()
            ...
            Frame:Remove()
            RealisticPoliceNotify(Realistic_Police.Lang[80][Realistic_Police.Langage])
        end 
    end
...
local function RealisticPolicePlate(ent, pos, ang, String)
    cam.Start3D2D( ent:LocalToWorld(pos), ent:LocalToWorldAngles(ang), 0.02 )
        ...
        if not Realistic_Police.PlateConfig[ent.Plate]["PlateText"] then 
            if ent:GetNWString("rpt_plate") != "" then 
                local StringExplode = string.Explode(" ", ent:GetNWString("rpt_plate")) or ""
                if istable(StringExplode) && isstring(StringExplode[1]) && isstring(StringExplode[2]) && isstring(StringExplode[3]) then 
                    draw.SimpleText(StringExplode[1]..StringExplode[2]..StringExplode[3], Realistic_Police.Fonts(RPTInformationVehicle[ent:GetModel()][String]["PlateSizeW"]/6),RPTInformationVehicle[ent:GetModel()][String]["PlateSizeW"]/Realistic_Police.PlateConfig[ent.Plate]["PlatePos"][1] - 5, RPTInformationVehicle[ent:GetModel()][String]["PlateSizeH"]/ Realistic_Police.PlateConfig[ent.Plate]["PlatePos"][2]+5, Realistic_Police.Colors["white"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER) 
                    draw.SimpleText(StringExplode[1]..StringExplode[2]..StringExplode[3], Realistic_Police.Fonts(RPTInformationVehicle[ent:GetModel()][String]["PlateSizeW"]/6),RPTInformationVehicle[ent:GetModel()][String]["PlateSizeW"]/Realistic_Police.PlateConfig[ent.Plate]["PlatePos"][1], RPTInformationVehicle[ent:GetModel()][String]["PlateSizeH"]/ Realistic_Police.PlateConfig[ent.Plate]["PlatePos"][2], Realistic_Police.PlateConfig[ent.Plate]["TextColor"], TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER) 
                end 
            end 
        else
            if ent:GetNWString("rpt_plate") != "" then 
...
hook.Add("Think", "RPT:Think", function()
    ...
    -- Can talk on the menu of the computer 
    if IsValid(RPTBaseFrame) then 
        if isstring(input.LookupBinding( "voicerecord" )) then 
            if input.IsKeyDown(input.GetKeyCode(input.LookupBinding( "voicerecord" ) )) then 
    ...
    if IsValid(LocalPlayer():GetEyeTrace().Entity) then 
        if LocalPlayer():GetEyeTrace().Entity:GetClass() == "realistic_police_screen" or LocalPlayer():GetEyeTrace().Entity:GetClass() == "realistic_police_policescreen" then 
            if LocalPlayer():GetEyeTrace().Entity:GetPos():DistToSqr((LocalPlayer():GetPos())) < 10000 then 
                ...
                draw.SimpleText(Realistic_Police.Lang[40][Realistic_Police.Langage], "Trebuchet24",ScrW()/2,ScrH()*0.97,Realistic_Police.Colors["white"],TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
...
hook.Add( "PlayerButtonDown", "RPT:PlayerButtonDownTrunk", function( ply, button )
	if button == (Realistic_Police.KeyForOpenTrunk or KEY_E) then 
        ...
        if not Realistic_Police.VehiclePoliceTrunk[ent:GetVehicleClass()] or not Realistic_Police.CanOpenTrunk[team.GetName(ply:Team())] then return end 
...
function Realistic_Police.NameCamera(CEntity)
    ...
    RPTLFrame.Paint = function(self,w,h)
        ...
        draw.DrawText(Realistic_Police.Lang[89][Realistic_Police.Langage], "rpt_font_7", RpRespX*0.004, RpRespY*0.005, Color(Realistic_Police.Colors["gray220"].r, Realistic_Police.Colors["gray220"].g, Realistic_Police.Colors["gray220"].b, RPTValue), TEXT_ALIGN_LEFT)
    end 
    ...
    PanelCT:SetText(" "..Realistic_Police.Lang[88][Realistic_Police.Langage])
    PanelCT:SetFont("rpt_font_7")
    PanelCT:SetDrawLanguageID(false)
    PanelCT.OnGetFocus = function(self) 
        if PanelCT:GetText() == " "..Realistic_Police.Lang[88][Realistic_Police.Langage] then  
            PanelCT:SetText("") 
        end
    end 
	...
	PanelCT.OnLoseFocus = function(self)
		if PanelCT:GetText() == "" then  
			PanelCT:SetText(" "..Realistic_Police.Lang[5][Realistic_Police.Langage])
		end
	end 
