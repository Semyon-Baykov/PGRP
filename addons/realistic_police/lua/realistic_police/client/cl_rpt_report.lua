...
function Realistic_Police.ReportMenu(RPTFrame, RPTable, RPTSendEntity, RPTIndex)
    ...
    DLabel2:SetText(Realistic_Police.Lang[14][Realistic_Police.Langage].." "..TimesDay)
    ...
    if not istable(RPTable) then 
        RPTText:SetValue(Realistic_Police.Lang[11][Realistic_Police.Langage])
    else 
    ...
    RPTDCombobox:SetText(Realistic_Police.Lang[15][Realistic_Police.Langage])
    ...
    RPTDCombobox:AddChoice( Realistic_Police.Lang[12][Realistic_Police.Langage] )
    ...
    function RPTDCombobox:OnSelect( index, text, data )
        ...
        if text == Realistic_Police.Lang[12][Realistic_Police.Langage] then 
            RPTEntity = nil
        else 
            RPTEntity = data
        end 
    ...
    if istable(RPTable) then 
        ...
        if IsValid(RPTSendEntity) then 
            ...
            RPTDCombobox:SetValue(Realistic_Police.Lang[12][Realistic_Police.Langage]) 
        end 
    ...
    RPTButtonSave:SetText(Realistic_Police.Lang[16][Realistic_Police.Langage])
    ...
    RPTButtonSave.DoClick = function()
        if Realistic_Police.JobEditReport[team.GetName(LocalPlayer():Team())] then 
            if IsValid(RPTEntity) then 
                if RPTEntity == LocalPlayer() then RealisticPoliceNotify(Realistic_Police.Lang[86][Realistic_Police.Langage]) return end 
            end 
            if RPTDCombobox:GetValue() == Realistic_Police.Lang[12][Realistic_Police.Langage] then 
                RPTEntity = nil
            end 
            if RPTActivate then 
                if not istable(RPTable) then 
            ...
            RealisticPoliceNotify(Realistic_Police.Lang[42][Realistic_Police.Langage])
        end 
    end
    ...
    RPTButtonDelete:SetText(Realistic_Police.Lang[13][Realistic_Police.Langage])
    ...
    RPTButtonDelete.DoClick = function()
        if Realistic_Police.JobDeleteReport[team.GetName(LocalPlayer():Team())] then 
            if IsValid(RPTEntity) then 
                if RPTEntity == LocalPlayer() then RealisticPoliceNotify(Realistic_Police.Lang[85][Realistic_Police.Langage]) return end 
            end 
            if istable(RPTable) then 
                if RPTDCombobox:GetValue() == Realistic_Police.Lang[12][Realistic_Police.Langage] then 
                    RPTEntity = nil
            ...
            RealisticPoliceNotify(Realistic_Police.Lang[41][Realistic_Police.Langage])
        end 
    end     
