                    ...
                    if istable(GetCameras(self:CPPIGetOwner())) then 
                        if IsValid( GetCameras(self:CPPIGetOwner())[self:GetNWInt("CameraId")] ) then 
                            if not GetCameras(self:CPPIGetOwner())[self:GetNWInt("CameraId")]:GetRptCam() then 
                                ...
                                draw.DrawText(Realistic_Police.Lang[43][Realistic_Police.Langage], "rpt_font_19", 113.5, 53, Realistic_Police.Colors["white"], TEXT_ALIGN_CENTER)
                                draw.DrawText(Realistic_Police.Lang[44][Realistic_Police.Langage], "rpt_font_19", 113.5, 73, Realistic_Police.Colors["red"], TEXT_ALIGN_CENTER)
                            end 
                        end  
                    end 
                end 
...
function ENT:CameraRender()
    ...
    hook.Add( "HUDPaint", "RPTCamera:Render"..self:EntIndex(), function()
        RPTPlayerDraw = true 
