...
SWEP.PrintName = "Выписыватель Штрафов"
		...
		if IsValid(Owner.WeaponRPT["Fine"]) then 
			...
			if Owner.WeaponRPT["Fine"]:IsPlayer() or Owner.WeaponRPT["Fine"]:IsVehicle() then 
				...
				if Owner:GetPos():DistToSqr(Owner.WeaponRPT["Fine"]:GetPos()) < 140^2 then 
					...
					if not Realistic_Police.JobCanAddFine[team.GetName(Owner:Team())] then 
						Realistic_Police.SendNotify(Owner, Realistic_Police.Lang[55][Realistic_Police.Langage])
						return 
					end 

					-- Check if the Player can have a fine 
					if Owner.WeaponRPT["Fine"]:IsPlayer() then 
						if Realistic_Police.JobCantHaveFine[team.GetName(Owner.WeaponRPT["Fine"]:Team())] then 
							Realistic_Police.SendNotify(Owner, Realistic_Police.Lang[56][Realistic_Police.Langage])
							return 
						end 
					end 
					...
					if Owner.WeaponRPT["Fine"]:IsVehicle() then 
						if Realistic_Police.VehicleCantHaveFine[Owner.WeaponRPT["Fine"]:GetVehicleClass()] then 
							Realistic_Police.SendNotify(Owner, Realistic_Police.Lang[57][Realistic_Police.Langage])
							return 
						end 
					end
