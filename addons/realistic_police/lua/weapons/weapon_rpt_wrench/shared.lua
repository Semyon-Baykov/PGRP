				...
				if Realistic_Police.CameraWorker[team.GetName(Owner:Team())] then 
					...
					timer.Create("rpt_camera_repair"..Owner:EntIndex(), Realistic_Police.CameraRepairTimer, 1, function()
						if IsValid(ply) then 
							...
							Realistic_Police.SendNotify(Owner, Realistic_Police.Lang[36][Realistic_Police.Langage].." "..DarkRP.formatMoney(Realistic_Police.CameraGiveMoney))
						end 
					end ) 
				end 
			end
