...
local V = {
	...
	Model = "models/lonewolfie/uaz_452.mdl", -- модель машины (в вкладке дополнения и проп авто)
...
if (file.Exists( "models/lonewolfie/uaz_452.mdl", "GAME" )) then -- путь модели (".mdl")
	list.Set( "simfphys_vehicles", "sim_fphys_uaz-452", V ) -- изменить на люброе название(например sim_fphys_lalala)
end

if CLIENT then 
	hook.Add("Think", "Simfphys_UAZ_452_Dash", function()
		for k,v in pairs(ents.FindByClass("gmod_sent_vehicle_fphysics_base")) do
			if v:GetModel() == "models/lonewolfie/uaz_452.mdl" then
				local speed = v:GetVelocity():Length()*0.12
				v:ManipulateBoneAngles(v:LookupBone("speed"), Angle(0, 0,-speed))
			end
		end
	end)
end