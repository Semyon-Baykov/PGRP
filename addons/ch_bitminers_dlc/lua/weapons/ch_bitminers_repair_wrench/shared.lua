...
if CLIENT then
	SWEP.PrintName			= CH_Bitminers_DLC.Config.Lang["Repair Wrench (Bitminers)"][CH_Bitminers_DLC.Config.Language]
	SWEP.Slot = 2
	SWEP.SlotPos = 1
	SWEP.DrawAmmo			= false
...
SWEP.Instructions 			= CH_Bitminers_DLC.Config.Lang["Left Click: Use repair wrench while aiming at a bitminer related entity."][CH_Bitminers_DLC.Config.Language]
...
function SWEP:PrimaryAttack()
	...
	-- Back to idle anim 76561198051291901
	timer.Simple( 0.2, function()
		if IsValid( self ) and IsValid( ply ) then
			if ply:GetActiveWeapon():GetClass() == "ch_bitminers_repair_wrench" then
				self:SendWeaponAnim( ACT_VM_IDLE )
	...
	if ( ply:GetPos() - tr.HitPos ):Length() < self.Primary.Range then
		...
		if tr.HitNonWorld then
			if SERVER then
				if table.HasValue( CH_Bitminers.ListOfEntities, ent:GetClass() ) then
					...
					if ent:Health() < ent:GetMaxHealth() then
						...
						DarkRP.notify( ply, 1, CH_Bitminers_DLC.Config.NotificationTime, CH_Bitminers_DLC.Config.Lang["This bitminer entity has reached it's maximum health!"][CH_Bitminers_DLC.Config.Language] )
						return
					end
					
					DarkRP.notify( ply, 1, CH_Bitminers_DLC.Config.NotificationTime, CH_Bitminers_DLC.Config.Lang["Bitminer entity has been healed."][CH_Bitminers_DLC.Config.Language] )
	...
	for k, ent in ipairs( ents.FindInSphere( LocalPlayer():GetPos(), CH_Bitminers_DLC.Config.ShowHealthDistance ) ) do
		...
		if not Trace.Hit then
			...
			if ent:Health() > 0 then
				draw.SimpleTextOutlined( CH_Bitminers_DLC.Config.Lang["Health:"][CH_Bitminers_DLC.Config.Language].. " ".. ent:Health(), "BITMINER_DLC_TabletSmaller", ScrPos.x, ScrPos.y - 65, prim_col, 1, 1, 1, outline_col )
			end	
		end
	end
end
