...
local function ramKeypad(ply, trace, ent)
	-- local tr = self.Owner:GetEyeTrace()
	-- local ent = tr.Entity
	
	if ply:EyePos():DistToSqr(trace.HitPos) > 5000 then return false end
	
	if IsValid(ent) and ent.IsKeypad then
...
local function getRamFunction(ply, trace)
    ...
    return
		...
		ent.IsKeypad		and fp{ramKeypad, ply, trace, ent} 						or
