DRP_Animations = DRP_Animations or {}

-- Load animations after the languages for translation purposes
hook.Add("loadCustomDarkRPItems", "loadAnimations", function()
    DRP_Animations[ACT_GMOD_GESTURE_BOW] = 'Поклон'
    DRP_Animations[ACT_GMOD_TAUNT_MUSCLE] = 'Танец 1'
    DRP_Animations[ACT_GMOD_TAUNT_DANCE] = 'Танец 2'
    DRP_Animations[ACT_GMOD_TAUNT_SALUTE] = 'Отдать честь'
    DRP_Animations[ACT_GMOD_GESTURE_BECON] = 'За мной'
    DRP_Animations[ACT_GMOD_TAUNT_LAUGH] = 'Смеяться'
    DRP_Animations[ACT_GMOD_TAUNT_CHEER] = 'Радоваться'
    DRP_Animations[ACT_GMOD_TAUNT_PERSISTENCE] = 'Поза льва'
    DRP_Animations[ACT_GMOD_GESTURE_DISAGREE] = 'Нет'
    DRP_Animations[ACT_GMOD_GESTURE_AGREE] = 'Палец вверх'
    DRP_Animations[ACT_GMOD_GESTURE_WAVE] = 'Помахать'
    DRP_Animations[ACT_GMOD_TAUNT_ROBOT] = 'Робот'
end)
...
if SERVER then
    local function CustomAnim(ply, cmd, args)
        ...
        if not DRP_Animations[Gesture] then return end
...
local function KeysDRP_Animations(um)
    local ply = um:ReadEntity()
    if not IsValid(ply) then return end
    local Type = um:ReadString()
    ply[Type] = true
end
usermessage.Hook("anim_keys", KeysDRP_Animations)
