local function updateAgenda(ply, agenda, text)
     ...
     if count > 5 then
        ...
        DarkRP.notify(ply, 1, 6, DarkRP.getPhrase("unable", "agenda", "Limit of the lines ( 5/5 )"))
...
local function addAgenda(ply, args)
    ...
    updateAgenda(ply, agenda, agenda.text .. ( string.Trim( agenda.text ) == '' and '' or '\n' ) .. args)
...
local function EnterLottery(answer, ent, initiator, target, TimeIsUp)
    ...
    if TimeIsUp then
        ...
        local mayor = ( #LotteryPeople >= 2 and IsValid( lcmayor ) and lcmayor:gp_VipAccess() ) and math.Round( ((#LotteryPeople * LotteryAmount)*5)/100 ) or 0
        ...
        if IsValid( lcmayor ) and lcmayor:gp_VipAccess() then
...
function DarkRP.lockdown(ply,re)
    local reason = re or 'Причина не указана'
    ...
    SetGlobalString("DarkRP_LockDownReason", reason)
    DarkRP.notifyAll(0, 3, 'Глава города объявил комендантский час. Причина: '..reason)
...
function DarkRP.unLockdown(ply)
    ...
    if not GetGlobalBool("DarkRP_LockDown", false ) then
    ...
    SetGlobalString("DarkRP_LockDownReason", "")
