...
surface.CreateFont( 'drphud_roboto', {
    Font = 'Roboto',
    size = 25
} )
local ConVars = {}
...
usermessage.Hook("GotArrested", function(msg)
    ...
    Arrested = function()
        ...
        if CurTime() - StartArrested <= ArrestedUntil and localplayer:getDarkRPVar("Arrested") then
            draw.SimpleTextOutlined(DarkRP.getPhrase("youre_arrested", math.ceil((ArrestedUntil - (CurTime() - StartArrested)) * 1 / game.GetTimeScale())), "drphud_roboto", Scrw / 2, Scrh - Scrh / 12, colors.white, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
...
---------------------------------------------------------------------------]]
net.Receive( 'darkrp_Notify', function( len, ply )
    ...
    notification.AddLegacy( txt, type, len )
